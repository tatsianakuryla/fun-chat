...
export class ChatFooterFactory {
  public static createContainer(): HTMLElement {
    const footerContainer = ContainerFactory.create('chat-footer');
    footerContainer.append(
      this._createFooterContent(),
      this._createDeveloperInfo(),
    );
    return footerContainer;
  }

  private static _createFooterContent(): HTMLElement {
    const schoolLink = createElementWithClassId('a', [
      style['chat__footer-link'],
      FLEX_CLASS,
    ]);
    schoolLink.href = 'https://rs.school/';
    schoolLink.target = '_blank';
    schoolLink.textContent = 'The Rolling Scopes School';

    const logoWrapper = createElementWithClassId('span', [
      style['chat__footer-logo'],
    ]);
    ...
    schoolLink.appendChild(logoWrapper);
    return schoolLink;
  }

  private static _createDeveloperInfo(): HTMLElement {
    const author = createElementWithClassId('div', [
      style['chat__footer-author'],
    ]);
    author.innerHTML = `© 2025 <a href="https://github.com/tatsianakuryla/" target="_blank">Tatsiana Kuryla</a>`;
    return author;
  }
}
